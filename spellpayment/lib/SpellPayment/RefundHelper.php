...
class RefundHelper
{
     ...
     *
     */
    public function processRefund(Order $order, array $productList, $payment_id)
    {
     ...
     * @return array
     */
    public function getRefundData(Order $order, $productList = [])
    {
     ...
     * @return float
     */
    public function getProductsRefundAmount($productList = array()) {
        $refundAmount = 0;
     ...
     * @return float
     */
    private function getShippingRefundAmount(Order $order) {
     ...
     * @return bool
     */
    public function isVoucherRefund() {
        $cancelProduct = Tools::getValue('cancel_product');
     ...
     * @return bool|string
     */
    public function isRefundAllow(Order $order, $productList = null) {
     ...
     * @throws \PrestaShopException
     */
    public function handleMessage(Order $order, $message)
    {
        OrderMessageHelper::addMessage($order, $message);
        list($configValues, $errors) = SpellHelper::getConfigFieldsValues();
        $spell = SpellHelper::getSpell($configValues);
        $spell->logInfo($message);
        $displayError = new Tools();
