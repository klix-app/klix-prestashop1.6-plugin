...
class spellpaymentpdpcheckoutModuleFrontController extends \ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (!\Configuration::get('PS_GUEST_CHECKOUT_ENABLED') && empty($this->context->customer->email)) {
        ...
        $checkout_url = isset($paymentRs['checkout_url']) ? $paymentRs['checkout_url'] : null;
        $spell_payment_uuid = isset($paymentRs['id']) ? $paymentRs['id'] : null;
        if (!$spell_payment_uuid) {
            $msg = 'Could not init payment in service - ' . json_encode($paymentRs);
            throw new \Exception($msg);
        }
        OrderIdToSpellUuid::addNew(array(
            'order_id' => $cart->id,
    ...
    {
        $cart = null;
        $id_product = isset($_REQUEST['product_id']) ? $_REQUEST['product_id'] : null;

        //id_product_attribute
        $id_product_attribute = isset($_REQUEST['id_product_attribute']) ? $_REQUEST['id_product_attribute'] : null;
    ...
    private function makePaymentParams($configValues)
    {
        ...
        $failure_url = $this->context->link->getModuleLink(
            $this->module->name,
            'checkoutcallback',
            array('cart_id' => $cart->id, 'restore_cart_id' => $cart->id)
        );
        $cancel_url = $this->context->link->getModuleLink(
            $this->module->name,
            'checkoutcallback',
            array('cart_id' => $cart->id, 'restore_cart_id' => $cart->id, 'is_cancel' => true)
        );
        $callback_url = $this->context->link->getModuleLink(
            $this->module->name,
            'checkoutcallback',
            array('cart_id' => $cart->id, 'is_module_callback' => true)
        );
        ...
        $params = array(
            ...
            'payment_method_whitelist' => array('klix')
        );
