...
{capture name=path}
	<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" title="{l s='Go back to the Checkout' mod='spellpayment'}">{l s='Checkout' mod='spellpayment'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='SpellPayment' mod='spellpayment'}
{/capture}

<h2>{l s='Order summary' mod='spellpayment'}</h2>
...
<h3>{l s='SpellPayment' mod='spellpayment'}</h3>
                ...
                <span class="payment-method-list">
                    {foreach $by_method item=data}
                        <label style="padding: 1em; width: 250px;">
                            <input type="radio" required="required" name="spell_payment_method" class="spell-payment-method"
                                value="{$data['payment_method']}" data-countries={$data['countries']|json_encode|escape} />
